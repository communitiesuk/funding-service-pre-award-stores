...
from pre_award.assess.assessments.models.round_summary import create_round_summaries
...
def mock_summaries_helpers(mocker):
    mocker.patch(
        "pre_award.assess.assessments.models.round_summary._add_links_to_summary",
        return_value=lambda summaries, fsn, r: summaries,
    )
    mocker.patch(
        "pre_award.assess.assessments.models.round_summary.has_devolved_authority_validation",
        return_value=False,
    )
...
def mock_live_stats(mocker):
    mock_populate_live_stats = mocker.patch(
        "pre_award.assess.assessments.models.round_summary._populate_live_round_stats",
...
def mock_assessment_stats(mocker):
    mock_populate_assessment_stats = mocker.patch(
        "pre_award.assess.assessments.models.round_summary._populate_assessment_stats",
...
def test_create_round_summaries_no_rounds(mocker, mock_live_stats, mock_assessment_stats):
    mocker.patch(
        "pre_award.assess.assessments.models.round_summary.determine_round_status",
        return_value=RoundStatus(False, False, False, False, False, False),
    )
    mocker.patch("pre_award.assess.assessments.models.round_summary.get_rounds", return_value=[])
...
def test_create_round_summaries_assessment_closed(
    ...
    with patch("pre_award.assess.assessments.models.round_summary.AssessmentAccessController") as mock:
