...
from tests.apply_tests.api_data.test_data import TEST_APPLICATIONS, TEST_ROUNDS_DATA
...
def mock_applications(mocker):
    mocker.patch("apply.default.application_routes.get_application_data", return_value=TEST_APPLICATION_EN)


def test_notification_route_after_deadline(flask_test_client, mocker, mock_login, mock_applications):
    mocker.patch("apply.default.application_routes.get_round_data_without_cache", return_value=TEST_ROUNDS_DATA[0])
...
def test_notification_route_before_deadline(flask_test_client, mocker, mock_login, mock_applications):
    mocker.patch("apply.default.application_routes.get_round_data_without_cache", return_value=TEST_ROUNDS_DATA[1])
    response = flask_test_client.get("/fund-round/notification/test-application-id", follow_redirects=False)
