...
def load_all_fund_rounds() -> None:
    ...
    loader_module_names = [
        f for f in os.listdir("pre_award/fund_store/scripts/fund_round_loaders") if f.endswith(".py")
    ]
    for module_name in loader_module_names:
        ...
        loader_module = importlib.import_module(
            f"pre_award.fund_store.scripts.fund_round_loaders.{module_name_without_extension}"
        )
