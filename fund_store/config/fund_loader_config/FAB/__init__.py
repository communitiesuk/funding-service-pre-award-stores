...
for file in os.listdir(this_dir):
    ...
    with open(this_dir / file, "r") as json_file:
        ...
        if content.startswith("LOADER_CONFIG = "):
            ...
            raise ValueError(
                f"fund config file {file.title()} does not start with 'LOADER_CONFIG='"
            )
        ...
        FAB_FUND_ROUND_CONFIGS[fund_short_name]["rounds"][round_short_name] = (
            loader_config["round_config"]
        )
        FAB_FUND_ROUND_CONFIGS[fund_short_name]["rounds"][round_short_name][
            "sections_config"
