...
def get_fund_and_round(fund_short_name: str, round_short_name: str) -> tuple[Fund, Round] | tuple[None, None]:
    fund = db.session.scalars(select(Fund).where(Fund.short_name == fund_short_name.upper())).one_or_none()
    if fund:
